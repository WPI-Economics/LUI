...
pc_data1$`Levelling-Up index` <- as.character(as.numeric(pc_data1$`Levelling-Up index`))

#this makes the hover over popup label
labels2 <- sprintf("<strong>%s</strong><br/>%s ranking<sup></sup>", pc_data1$pcon19nm.x, pc_data1$`Levelling-Up index`) %>% lapply(htmltools::HTML)
...
#The map oblect
my.map1 <- leaflet() %>% setView(-1.161877,52.350304, 6) %>% #setView gives centre coordinates and zoom level
...
#The map oblect
my.map2 <- leaflet() %>% setView(-1.161877,52.350304, 6) %>% #setView gives centre coordinates and zoom level
  setMapWidgetStyle(list(background = "white")) %>%  
...
#The map oblect
my.map3 <- leaflet() %>% setView(-1.161877,52.350304, 6) %>% #setView gives centre coordinates and zoom level
  setMapWidgetStyle(list(background = "white")) %>%  
...
#The map oblect
my.map4 <- leaflet() %>% setView(-1.161877,52.350304, 6) %>% #setView gives centre coordinates and zoom level
  setMapWidgetStyle(list(background = "white")) %>%  
...
#The map oblect
my.map5 <- leaflet() %>% setView(-1.161877,52.350304, 6) %>% #setView gives centre coordinates and zoom level
  setMapWidgetStyle(list(background = "white")) %>%  
...
#The map oblect
my.map6 <- leaflet() %>% setView(-1.161877,52.350304, 6) %>% #setView gives centre coordinates and zoom level
  setMapWidgetStyle(list(background = "white")) %>%  
...
#The map oblect
my.map7 <- leaflet() %>% setView(-1.161877,52.350304, 6) %>% #setView gives centre coordinates and zoom level
  setMapWidgetStyle(list(background = "white")) %>%  
...
print(my.map1)
###########################################################################
## OPTION 1: TRYING OUT STORYBOARDS ##
