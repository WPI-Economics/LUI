...
colnames(combi) <- c("pcon19cd", "pcon19nm", "Spending power", "Dependency", "Crime", "Deprivation", "Health", "Empty commercial properties", "Levelling-Up index")
...
#this makes the hover over popup label
labels2 <- sprintf("<strong>%s</strong><br/>%s score<sup></sup>", pc_data2$pcon19nm.x, round(pc_data2$`Spending power`,0)) %>% lapply(htmltools::HTML)
...
#this makes the hover over popup label
#this makes the hover over popup label
labels3 <- sprintf("<strong>%s</strong><br/>%s score<sup></sup>", pc_data3$pcon19nm.x, round(pc_data3$Dependency,0)) %>% lapply(htmltools::HTML)
...
#this makes the hover over popup label
labels4 <- sprintf("<strong>%s</strong><br/>%s score<sup></sup>", pc_data4$pcon19nm.x, round(pc_data4$Crime,0)) %>% lapply(htmltools::HTML)
...
#labels
labels5 <- sprintf("<strong>%s</strong><br/>%s score<sup></sup>", pc_data5$pcon19nm.x, round(pc_data5$Deprivation,0)) %>% lapply(htmltools::HTML)
...
#this makes the hover over popup label
labels6 <- sprintf("<strong>%s</strong><br/>%s score<sup></sup>", pc_data6$pcon19nm.x, round(pc_data6$Health,0)) %>% lapply(htmltools::HTML)
...
#this makes the hover over popup label
labels7 <- sprintf("<strong>%s</strong><br/>%s score<sup></sup>", pc_data7$pcon19nm.x, round(pc_data7$'Empty commercial properties',0)) %>% lapply(htmltools::HTML)
...
A higher score is better on the ‘Spending power’ indicator. The England and Wales average is 100. This indicator is comprised of employment and wage data to show a constituency’s economic strength. 
...
A lower score is better on the ‘Financial Dependency’ indicator. The England and Wales average is 100. This indicator uses Job Seekers’ Allowance, Universal Credit claims, and skills rates to show an area’s current and likely dependence on state support.
...
A lower score is better on the ‘Crime’ indicator. The England and Wales average is 100. This indicator is simply the crime rates for each area.
...
A lower score is better on the ‘Deprivation’ indicator. The England average is 100. This indicator is based on the deprivation score across seven metrics. 
...
A higher score is better on the ‘Health’ indicator. The England average is 100. This indicator is based on both the number of GPs and the average life expectancy for an area.
...
A lower score is better on the ‘Empty commercial properties’ indicator. The England and Wales average is 100. This indicator is based on commercial vacancy rates, i.e. the number of empty shops in a constituency. 

Seats in the North and Wales record lower quality of place scores with Grimsby, Newport and Blackpool amongst the highest. They record scores almost twice the national average. 29 out of the top 30 highest scoring seats are in the North, Midlands, or Wales. Croydon Central is the only southern seat in the top 30. 

Areas in the South and East have better quality of place scores with Forest of Dean, Mid Bedfordshire, South Cambridgeshire and Rayleigh and Wickford recording scores at half the national average.
